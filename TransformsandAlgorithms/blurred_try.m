...
%X1=.8;
for X1=0:.0158:1
    F(p)=(1/sqrt(2*pi*(.6^2)))*exp(-((X1^2)/2*(.6)^2));
    p=p+1;
end
p=1;
G=zeros(64,64);
...
for jj = 1:10000:size(blocks,2)
   ...
   Coefs= OMPerr(G1,VV,D,blocks(:,jj:jumpSize),errT); %% size of Coefs:-64 by 15625
   
 if (Reduce_DC)
        blocks1(:,jj:jumpSize)=d*Coefs + ones(size(blocks1,1),1) * vecOfMeans;
    else
...
while errT>.002
 for jj = 1:10000:size(blocks1,2)
   ...
   Coefs1= OMPerr_old(d,blocks1(:,jj:jumpSize),errT); %% size of Coefs:-64 by 15625
