...
%  load kanchan1
%  load sampling_matrix2_16_64_DCT;
%  currDictionary=DICTIONARY;   
 qwe=im2col(original,[8 8],'sliding');
size(qwe)
%  Alpha1=OMP(currDictionary,qwe,25);
%%
 
%  norm_phi=sqrt(sum(phi.^2,1));
% phi=phi./repmat(norm_phi,size(phi,1),1);
%  
phi=zeros(16,64);
n=randperm(64);
for i=1:19
...
[rows,cols] = ind2sub(size(idxMat),idx);
 ...
 IOut = IMout1./(Weight);
% % % %  I=backprojection(I,InputImg,20);
  I=IOut;
% I=im2uint8(I);
% I=im2uint8(I);
